...
def _field(readings: dict[str, IndicatorReading], indicator: str, field: str):
    if indicator not in readings:
        return None
    return readings[indicator].detail.get("fields", {}).get(field)


def _recent_values(series_id: str, n: int) -> list[float]:
...
def _recent_pct_changes(series_id: str, periods: int, n: int) -> list[float]:
    df = collector_base.read_normalized(series_id)
    if df.empty:
        return []
...
def _bp_change_row_offset(series_id: str, offset_rows: int) -> float | None:
...
def _compute_signals(readings: dict[str, IndicatorReading]) -> dict[str, bool]:
    ...
    exports_yoy = _field(readings, "exports", "yoy")
    semi_yoy = _field(readings, "semiconductor_exports", "yoy")
    ...
    ppi_yoy_hist2 = _recent_pct_changes("ecos_ppi_yoy_level", 12, 2)
    s["ppi_easing"] = len(ppi_yoy_hist2) == 2 and (ppi_yoy_hist2[0] - ppi_yoy_hist2[1]) >= t["ppi_easing_drop_pp"]

    fx_mom = _recent_pct_changes("ecos_usdkrw", 1, 1)
    s["fx_surge"] = bool(fx_mom) and fx_mom[-1] >= t["fx_surge_mom_pct"]

    rate_bp_3m = _bp_change_row_offset("ecos_kr_3y_yield", 63)
    ...
    gdp_qoq = _field(readings, "gdp", "qoq")
...
def _persistence_upgrade(current_regime: str, history: list[dict], raw_score: float,
                          readings: dict[str, IndicatorReading]) -> str | None:
    """11.7 example: Early Expansion -> Expansion when score improves 2 months running
    and exports/production co-improve."""
    if current_regime != "Early Expansion" or len(history) < 1:
        return None
    prev = history[-1]
    prev_score = prev.get("macro", {}).get("scores", {}).get("raw_score")
    if prev_score is None or raw_score <= prev_score:
        return None
    if len(history) >= 2:
        prev_prev_score = history[-2].get("macro", {}).get("scores", {}).get("raw_score")
        if prev_prev_score is not None and prev_score <= prev_prev_score:
            return None
    exports_ok = (readings.get("exports").score or -1) >= 0 if "exports" in readings else False
    production_ok = (readings.get("industrial_production").score or -1) >= 0 if "industrial_production" in readings else False
    if exports_ok and production_ok:
        return "Expansion"
    return None
...
def determine_regime(readings: dict[str, IndicatorReading], raw_score: float, history: list[dict]) -> dict:
    ...
    previous_regime = history[-1]["macro"]["regime"] if history and history[-1].get("macro", {}).get("regime") else seed_regime
    ...
    if len(downgrade_hits) >= cfg["downgrade"]["trigger_count"]:
        ...
        promoted = _persistence_upgrade(previous_regime, history, raw_score, readings)
        if promoted:
    ...
    return {
        ...
        "warnings_kr": [SIGNAL_LABELS_KR.get(k, k) for k in warning_hits] if warning_active else [],
        "all_signals": signals,
    }
