...
from . import base
from .kr_regions import HIGHLIGHT_REGION, REGION_TIERS, SEOUL_DISTRICTS, TIER_LABELS, all_regions
...
def _fetch_region_month(lawd_cd: str, deal_ymd: str, api_key: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        # data.go.kr's standard OpenAPI error envelope comes back as XML even when type=json is
        # requested — see collectors/molit.py's _fetch_region_month for the full explanation.
        raise RuntimeError(
            f"MOLIT villa (연립다세대) returned non-JSON response (likely a service/auth error): "
    ...
    if header.get("resultCode") not in (None, "00", "000"):
        raise RuntimeError(f"MOLIT villa (연립다세대) error response: {header.get('resultMsg')}")
