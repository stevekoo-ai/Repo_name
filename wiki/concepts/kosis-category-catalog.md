...
| TBL_ID | TBL_NM | 비고 |
|---|---|---|
| DT_1F02001 | 시도/산업별 광공업생산지수(2020=100) | industrial_production_index 유력 후보 — 산업별 breakdown에 반도체(KSIC C26 계열)가 있을 가능성, item-meta 미실행 |
...
**다음 시도할 것**: `DT_1F02001`(시도/산업별)에 item-meta 적용해 "산업별"
분류(OBJ_ID) 밑에 반도체(KSIC C26 전자부품ㆍ컴퓨터ㆍ영상ㆍ음향및통신장비,
또는 그 하위 반도체 세분류) ITM_ID가 있는지 확인 — 이게 있으면
industrial_production_index(전산업 합계 ITM)와 semiconductor_shipment/
inventory_index(반도체 세부 ITM, 단 이 표는 "생산지수"라 출하/재고는 아직
별도 표가 필요할 수 있음)를 이 표 하나에서 커버할 가능성.
