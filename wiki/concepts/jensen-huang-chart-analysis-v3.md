---
name: jensen-huang-chart-analysis-v3
description: NVIDIA GTC Inference Economics 차트 v2 + CXL 3.2 PCIe Gen6 100TB Pooled DRAM 확장 분석 — 용량의 벽 허물기와 x=800 초가혹 용량 계획 시나리오
metadata:
  type: reference
  created: 2026-08-26
  verified: 2026-08-26
  supersedes: jensen-huang-chart-analysis-v2.md (2026-08-26, v2 내용 전체 포함 보존)
tags: [jensen-huang, gtc, inference, gpu, chart-analysis, ro-v3, blackwell, rubin, hopper, cxl, pooled-memory, capacity-planning]
---

# 젠슨황 "Inference Economics" 차트 — v2 + CXL 풀드 메모리 확장판 (v3)
...
>
> **v3가 만들어진 이유**: v2(2026-08-26)는 12포인트 판독·물리 역산·세 곡선 해석을 확정했으나,
> **CXL 3.2 PCIe Gen6 100TB Pooled DRAM**이 이 차트에 주는 영향을 다루지 않았다. v3는 v2의
> 전체 내용을 보존한 채, CXL 풀드 메모리 확장(기본 분석 + x=800 초가혹 용량 계획 시나리오)을
> 추가한다. **v2는 수정 없이 보존** — 본 파일은 v2 내용 전체 + CXL 섹션(13~15)으로 구성된다.
>
> **생성일**: 2026-08-26 · **검증일**: 2026-08-26
> **관련 문서**: [gpu-inference-economics.md](gpu-inference-economics.md) / [gpu-inference-chart-verification.md](gpu-inference-chart-verification.md) / [jensen-huang-chart-analysis-v2.md](jensen-huang-chart-analysis-v2.md) (v2 원본 보존)
...
| GPU/rack | **32** | 72 (4개/server × 18 server) | 72 |
...
| NVL72 HBM BW 총합 | (32 GPU) 107.2 TB/s | **460.8 TB/s** (80% 효율) | **1,584 TB/s** |
| NVLink | 4th gen (900 GB/s/GPU) | **5th gen (130 TB/s 패브릭)** | — |
| System Memory | DDR5 8TB | **LPDDR5X 18TB (16.6 TB/s)** | SOCAMM 80TB + CXL 100TB |
| 냉각 | 공랭 가능 | 100% 수랭 필수 | DLC 필수 |
...
| [jensen-huang-chart-analysis-v2.md](jensen-huang-chart-analysis-v2.md) | **v2 (2026-08-26, 본 v3의 베이스, 보존됨)** |
...
# ▼▼▼ v3 신규 추가 — CXL 3.2 PCIe Gen6 100TB Pooled DRAM 확장 분석 ▼▼▼

## 13. CXL 풀드 메모리 기본 분석 — 용량의 벽 허물기

> **v3 출발점**: 사용자 전제 "CXL 3.2 PCIe Gen6, 100TB 추가 DRAM"이 기존 12포인트
> 차트(x=50~400)에 주는 변화를 정량. 핵심 명제부터 세운다.

### 13.1 핵심 명제 — CXL은 "용량 자원"이지 "대역폭 자원"이 아니다

Jensen 차트의 두 축이 각각 지배받는 자원:

| 축 | 의미 | 지배 자원 |
|---|---|---|
| **Y = TPS/MW** | 전력당 처리효율 | **대역폭 + 연산** (HBM BW, FP4 POPS) |
| **X = TPS/user** | 인터랙티비성(1인당 속도) | **용량 + 대역폭** (KV cache 크기, attention BW) |

**CXL 3.2 PCIe Gen6 100TB Pooled DRAM 물리적 성질 [추정]**:

| 항목 | 값 [추정] | HBM4(R100 22TB/s) 대비 |
|---|---|---|
| **용량** | **100 TB** | HBM(20.3TB 가용)의 **~5배** — 압도적 용량 |
| **대역폭** | 랙당 ~2–4 TB/s (Gen6 x16 ~126 GB/s/포트 × 다포트) | HBM4 22TB/s의 **~10–18%** — 한참 느림 |
| **지연** | ~250–400ns | HBM ~100–150ns의 **~2–3배** |
| **전력** | ~8 kW (DDR5 100TB ~4.7kW + 컨트롤러/스위치 ~3kW) | R100 199kW 대비 +4% |

> **결론 한 줄**: **CXL은 용량의 벽은 허물지만, 대역폭의 지붕은 올리지 못한다.**
> Jensen 차트의 *upper envelope*(곡선 최고점)은 GPU+HBM이 정하고, CXL은 *N/A 영역을
> 채우고 절벽을 경사로 만드는* 역할만 한다.

### 13.2 세 GPU 랙별 기본 변화 (CXL 100TB +8kW 추가, v2 → v3)

#### 🔴 H100 랙 (32 GPU, 40kW → **48kW**, +20%)

| 항목 | v2 | v3 |
|---|---|---|
| GPT 2T 가중치 4,000GB vs HBM 2,560GB | **N/A(회색선)** | CXL 100TB가 가중치 4TB + KV 모두 흡수 → **N/A → 지원 가능(저 TPS)** |
| 절벽 형태 | x=200부터 점 없음 | 절벽이 **꼬리(저-TPS 실선)**로 살아남 |
| 병목 | 용량 부족 | 가중치 매 토큰 CXL→HBM 스트리밍: 4TB / 4TB/s ≈ 1s/배치 → **weight-bound** |
| 전력 패널티 | — | TPS 그대로면 ×(40/48) = **0.83×** → x=50/100(기존 지원)은 ~20% ↓, x=200/400은 0→(작지만 양수)로 **대승** |

#### 🟡 B300 랙 (72 GPU, 121kW → **129kW**, +6.6%)

| 항목 | v2 | v3 |
|---|---|---|
| binding constraint | HBM3e **8TB/s 대역폭** (용량 아님) | 동일 — CXL은 쓸 곳 없음 |
| CXL 효용 | — | **순수 데드웨이트**: CXL 대역폭(4TB/s) < HBM(8TB/s) → KV를 CXL로 빼면 *오히려 느려짐* |
| 결과 | — | 전력만 +6.6% → 전 x 구간 TPS/MW **~7% ↓** |
| 예외 | — | x>400(400K 초과 초장문) **새 영역**만 CXL이 열어준다. 현 차트 x축 범위 내에서는 **손해** |

#### 🟢 R100 랙 (72 GPU, 199kW → **207kW**, +4%)

| 항목 | v2 | v3 |
|---|---|---|
| spill(v2) | x=50/100/200 spill → SOCAMM 80TB @ 44TB/s + C2C 1.8TB/s 방어 | CXL이 SOCAMM을 **대체** vs **추가**? |
| CXL이 SOCAMM 대체 시 | SOCAMM 44TB/s | CXL 4TB/s → spill 흡수는 되지만 *spill된 KV 접근이 11× 느려짐* → R100 x=50/100 TPS는 v2보다 **오히려 하락** |
| CXL을 SOCAMM 위에 추가 시 | 80TB | 180TB 용량 과잉, 의미 없음 |
| 현실적 해석 | — | CXL 3.2는 *표준화된 scale-out tier*로 SOCAMM(독자)을 표준으로 교체 방향. 그렇다면 spill은 해소되지만 **spill 대역폭 44TB/s → 4TB/s로 축소** → 활성 KV 서빙 페널티 |
| 정량 | — | spill 페널티 제거로 TPS +10~20% vs 대역폭 저하 −5~15% vs 전력 −4% → **net ±0~+10%, 불확실**. v2 R100 "수렴(Roofline)" 곡선은 **CXL 대역폭이 새 벽**이 되어 *오히려 낮아질 수도* |

### 13.3 차트 형태 변화 (v2 → v3, x=50~400 구간)

```
Y(TPS/MW)
 │  v2:  절벽(H100)  하강(B300)  수렴(R100)
 │
 │ R100 ──────────────●────●────●────●      ← upper envelope 거의 불변
 │        v3: CXL 추가로 약간 ↓(전력) or ↓(BW 벽)
 │ B300 ───────●────●────●────●              ← 전 구간 ~7% ↓(데드웨이트)
 │ H100 ──●────●────────╱N/A╲──────────      ← v2: x≥200 회색선
 │        v3: ●────●────╲──●────●── (저TPS)   ← 절벽이 꼬리로 채워짐
 │                          N/A → 실선(저)
 └────────────────────────────────────→ X(TPS/user)
        x=50   100   200   400   (x>400 새 영역)
```

**4가지 형태 변화 정리**:

1. **절벽 → 경사**: H100 x≥200 회색선(N/A)이 저-TPS 실선으로 채워짐. 용량 벽 허물어짐.
2. **upper envelope 불변**: 곡선 최고점(R100 1.65M, B300 0.7M)은 GPU+HBM이 정하고, CXL은 못 올림. Jensen "세대별 효율 우위" 메시지 **유지**.
3. **전체 곡선 미세 하강**: CXL 전력 오버헤드(+4~20%)로 Y값 전반 ↓. B300이 가장 크게 손해(데드웨이트).
4. **x축 확장 가능**: x>400(400K 초과 초장문)이 B300/H100+CXL로도 서비스 가능 → 새 티어(예: x=800, 1M context) 등장 여지.

### 13.4 가장 중요한 통찰 — Jensen vs CXL **서사 충돌**

이것이 v3 기본 분석의 본질:

- **Jensen 차트의 목적**: "HBM이 모자라면 → 다음 GPU 세대 사라(B300→R100)." 절벽/N/A가 *GPU 업그레이드 영업 도구*.
- **CXL의 목적**: "HBM 용량 부족? → 싼 DRAM 100TB 풀면 된다." 절벽을 *저렴하게 평탄화*.
- **충돌**: CXL은 Jensen이 *파는데 쓰는 절벽*을 메운다. x=400($45 Premium)이 R100만 가능하던 것을 → H100/B300 + CXL도 (느리지만) 서빙 가능 → **티어별 가격 격차($0/$3/$6/$45) 압축 압력**.

→ **CXL은 GPU 업그레이드 긴급성을 *부분적으로 중화*하는 기술.** 차트의 upper envelope(효율)엔 손 안 대지만, "용량 때문에 무조건 R100"이라는 논리를 "용량은 CXL, 효율만 R100"으로 분리.

> **v3 핵심 명제**: **"CXL은 효율의 지붕은 안 올리고, 용량의 절벽을 경사로 깎는다."**
> x=50~400 현 차트 구간에서는 CXL의 한계(BW < HBM) 때문에 주인공이 되지 못하고,
> **x=800 초가혹 구간(Section 14)에서야 비로소 잉여→생사줄로 전위**한다.

---

## 14. x=800 초가혹 용량 계획 시나리오 — CXL이 생사줄이 되는 지점

> ⚠️ **프레임 명시 (필독)**: 본 섹션은 **용량 계획(Capacity Planning)** 시나리오다.
> "목표 동시 사용자 수(예: 500명)를 1M 컨텍스트 5T 모델로 서빙할 수 있는가?"라는
> 질문에 답한다. 이는 v2 차트의 **"x=800에서 효율 곡선이 어디로 가는가?"** 질문과
> **다르다**:
> - **v2 차트 추세론**: x↑ → TPS/MW ↓(곡선 하강) → 총 TPS ↓ → 수용 유저 ↓ → **spill 자체 소멸**. x=800에서 R100은 ~10–50명만 서빙, KV 3–13TB, spill 없음.
> - **용량 계획론(본 시나리오)**: 목표 유저(500명)를 고정 → KV 128TB 강제 → HBM 초과 → **spill 필연, CXL 생사줄**.
>
> 즉 "파멸적 spill"은 **높은 동시성을 전제로 할 때만** 발생한다. 본 시나리오는 이 전제를
> 명시적 **stress-test**로 다룬다 — v2 "곡선 하강" 논리와 모순되지 않는 별개 프레임.

### 14.1 x=800 초가혹 환경 물리량 (독립 derive 검증, [정정] 1건)

**대상**: [가상] GPT-5급 5조(5T) 파라미터 Dense/MoE 모델 · 컨텍스트 1,000K(100만 토큰)

```
가중치(Weights):
  5T params × FP8(1 byte) = 5,000 GB (5 TB)     ← 채택 (사용자 전제)
  5T params × NVFP4(0.5 byte) = 2,500 GB         ← [정정] R100 가중치 기준
     ※ 사용자 제시 1,250GB는 2-bit 양자화(비표준). NVFP4는 4-bit = 0.5 B/param.
     ※ 파급: R100 가용 HBM 19.5TB(제시) → 18.2TB(정정). 결론 불변.

KV per user (1M ctx, 256 KB/token):
  256 KB × 1,000,000 / 1e6 = 256 GB/user          ← 정확

HBM (단일 랙):
  H100: 80GB × 32 = 2,560 GB (2.5 TB)
  B300/R100: 288GB × 72 = 20,736 GB (20.7 TB)

가용 HBM (weights 차감 후):
  H100:  2,560 − 5,000 = 음수 → 가중치 자체 못 담음 → 사망
  B300:  20,736 − 5,000(FP8) = 15,736 GB = 15.7 TB
  R100:  20,736 − 2,500(FP4) = 18,236 GB = 18.2 TB   ← [정정] (제시 19.5 → 18.2)

HBM-only 최대 동시 유저:
  B300: 15,736 / 256 = 61.5명  ← "60명 고갈" 주장 검증 OK
  R100: 18,236 / 256 = 71.2명  ← [정정] (제시 78 → 71)

H100 사망 검증:
  HBM 2.56TB < 가중치 5TB → 가중치 자체 불가.
  PCIe Gen5 64GB/s로 5TB 스트리밍 = 78초/패스 → 0 TPS 고정. CXL로도 구제 불가.
```

### 14.2 용량 계획 시나리오 — 목표 동시 사용자 500명 (KV 128TB 강제)

> **전제**: 500명 × 256GB/user = 128TB KV 요구. HBM-only 한계(B300 61, R100 71) 초과 →
> **spill 필연**. 이때 CXL 100TB(+R100 SOCAMM 80TB)가 결정적.

| 랙 | 가용 HBM | HBM-only 한계 | 500명 KV=128TB → spill | CXL 100TB(+SOCAMM) 후 수용력 | CXL 역할 |
|---|---|---|---|---|---|
| **H100** | 음수(가중치 불가) | 0명 | **N/A (사망)** | Gen5 64GB/s 스트리밍 불가 → **0 TPS 고정** | 🚫 구제 불가(안락사) |
| **B300** | 15.7TB | 61명 | 112TB spill | (15.7+100)/0.256 = **~452명** | 🔵 산소호흡기(생존) |
| **R100** | 18.2TB [정정] | 71명 | 110TB spill | (18.2+80+100)/0.256 = **~770명** | 🟢 Scale-out 핵심 무기 |

**정합성 (독립 derive 1:1 대조)**:
- 가중치 FP8 5,000GB ✓ · KV/유저 256GB(1M ctx) ✓
- B300 가용 15.7TB ✓ · "60명에 HBM 고갈"(61.5명) ✓
- H100 2.56TB < 5TB 가중치 → 사망 ✓ · Gen5 64GB/s 스트리밍 78s/패스 → 0 TPS ✓
- R100 FP4 가중치 **2,500GB** [정정] · 가용 18.2TB · HBM-only 71명
- R100 수용력 770명 = B300 452명의 **1.7배** → 독보적 방어선 유지

### 14.3 세대별 CXL 구원 메커니즘 (정정된 수치 반영)

#### ① Hopper H100 — 영원한 안락사 (CXL로도 구제 불가) 🚫

```
가중치 5TB > H100 HBM 2.56TB (2배 초과).
CXL 100TB 풀에 가중치+KV 전부 배치해도, H100의 PCIe Gen5(64GB/s)로는
5TB 가중치를 실시간 읽어올 수 없음 (78s/패스).
→ CXL 풀을 아무리 달아도 H100 선은 완벽히 0 TPS 영역에 고정. 부활 불가.
```

#### ② Blackwell B300 — '전사(OOM)'에서 '실선 가동(Feasible)'으로 극적 부활 📉

```
CXL 장착 전: B300 자체 HBM(15.7TB)으로 500명 KV(128TB) 감당 불가 → OOM 다운 →
             그래프 선 단절(0 TPS 절벽).
CXL 장착 후: 100TB CXL 풀이 넘치는 112TB spill을 흡수(VRAM 확장 영토).
             → 절벽이 끊겼던 B300 선이 x=800에서 유효 실선 포인트로 극적 부활.
             단, PCIe/CXL 경로(4TB/s) 속도 한계로 지연 패널티 강하게 받아
             곡선이 바닥 향해 가파르게 흘러내림. (느리지만 구동 상태 확보)
수용력 상한: ~452명 (CXL 100TB 1개 기준). 500명 초과 시 CXL 2-pool 또는 R100 이관 필요.
```

#### ③ Vera Rubin R100 — CXL 패브릭 통한 '진정한 가성비 곡선 사수' 🚀

```
CXL 장착 전: R100도 FP4 가중치(2.5TB) 차감 후 18.2TB 가용. 71명까지만 HBM 수용.
             500명 KV 128TB → 110TB spill. SOCAMM 80TB마저 꽉 차서 무너지기 시작.
             기울기 급격히 꺾여 내려가는 지점.
CXL 장착 후: R100은 차세대 CXL 3.0/4.0 대규모 패브릭 스위치를 네이티브 제어.
             100TB 외부 풀드 메모리를 자기 HBM처럼 오버헤드 없이 초고속
             다이렉트 주소 할당(Coherent Pooling) 수행.
             → x=800 초장문/초대형 가혹 조건에서도 절벽으로 떨어지거나 가파르게
               무너지지 않고, CXL 풀 디딤돌 삼아 완만 우하향하며 독보적 고효율
               TPS/MW 방어선 유지.
수용력: ~770명 (HBM 18.2TB + SOCAMM 80TB + CXL 100TB). B300의 1.7배.
```

### 14.4 엔지니어링 밸런스 결론 (x=800 가혹 시나리오)

```
X축이 x=800으로 연장되는 가혹 미래 시나리오에서, CXL 풀드 메모리는 더 이상
'B300에서 미동도 안 하던 잉여 장비'가 아니다.

  B300에게는: 시스템이 켜지지도 못하고 죽는 것을 막아주는 '산소호흡기(생존의 도구)'.
  R100에게는: 단일 랙 경계 넘어 DC 전체 자원 한 몸처럼 엮어 압도적 효율 유지하게
              만드는 '아키텍처 확장(Scale-Out)의 핵심 무기'.

즉, X축 연장될수록 그래프는 CXL 풀 장착 순간
  "H100은 사망, B300은 가파른 생존 곡선 부활, R100은 CXL 패브릭 기반 독보적 완만 하강 방어 곡선"
으로 완벽히 재정의.

계층 순위 R100 ≫ B300 ≫ H100 불변 — 사용자 정성 서사 100% 유효.
R100 수용력 770명 = B300 452명의 1.7배로 독보적 방어선 유지 확인.
```

### 14.5 문서에 명시한 3건 (투명성 라벨)

1. **R100 FP4 = 2,500GB** [정정] — NVFP4 4-bit 기준 (사용자 제시 1,250GB의 2배)
2. **프레임 라벨** — 본 시나리오는 용량 계획(목표 동시성 500명) 기준이며, v2 차트의
   효율 곡선 추세(높은 x에서 수용 유저 감소)와는 별개의 가혹 전제(stress-test)
3. **B300 수용력 상한 ~452명** — 100TB CXL 1개 기준; 500명 초과 시 CXL 2-pool 또는 R100 이관 필요

---

## 15. v3 종합 결론 — CXL 풀드 메모리의 두 얼굴

```
┌─────────────────────────────────────────────────────────────────┐
│  v3 핵심 명제: CXL 3.2 100TB는                                   │
│  "효율의 지붕은 안 올리고, 용량의 절벽을 경사로 깎는다."           │
│                                                                  │
│  구간별 두 얼굴:                                                  │
│                                                                  │
│  [x=50~400 현 차트]  → CXL은 잉여/손해 구간 (대역폭 < HBM)        │
│    - H100: 절벽 꼬리로 채움 (N/A→저TPS)                          │
│    - B300: 순수 전력 손해 ~7% (데드웨이트)                       │
│    - R100: SOCAMM 대체 시 오히려 spill 대역폭 축소 → 하락 가능    │
│    - upper envelope(R100>B300>H100) 불변                         │
│                                                                  │
│  [x=800 초가혹 확장]  → CXL이 생사줄로 전위 (용량 5배 압도)       │
│    - H100: 사망 (Gen5 64GB/s로 5TB 못 끌어옴)                     │
│    - B300: 산소호흡기 (OOM→부활, 수용력 452명)                    │
│    - R100: Scale-out 핵심 무기 (수용력 770명 = B300 1.7배)        │
│                                                                  │
│  Jensen vs CXL 서사 충돌:                                        │
│    - Jensen: 절벽 = GPU 업그레이드 영업 도구                     │
│    - CXL: 절벽을 싼 DRAM으로 평탄화 → 가격 격차 압축 압력        │
│    - 정리: "용량은 CXL, 효율은 R100"으로 분리                     │
└─────────────────────────────────────────────────────────────────┘
```

**v3 검증 기준**:
- v2 12포인트 전체 보존 (수정 0건) — v2는 [jensen-huang-chart-analysis-v2.md](jensen-huang-chart-analysis-v2.md)에 그대로 유지
- x=800 물리량 독립 derive 1:1 대조 완료 (1건 정정: R100 FP4 1,250→2,500GB)
- 라벨 무결성: CXL BW/C2C/SOCAMM = 2026 추정치 · Kimi 84.38, GPT 256 KB/tok = 추정치 (공식 비공개) · R100 FP4 가중치 [정정] 명시

---

> **v3 완료 선언**: v2의 확정된 12포인트 판독·물리 역산·세 곡선 해석을 전체 보존한 위에,
> CXL 3.2 PCIe Gen6 100TB Pooled DRAM이 차트에 주는 영향을 두 프레임(기본 분석 +
> x=800 초가혹 용량 계획)으로 정량·검증했다. 핵심 — **CXL은 x=50~400 현 차트에서는
> 잉여/손해 구간이나, x=800 초가혹 구간에서야 비로소 생사줄로 전위**한다. 이것이
> "용량은 CXL, 효율은 R100"으로 GPU 업그레이드 논리를 분리하는 v3의 결론이다.
> 이후 모든 Jensen Chart + CXL 논의는 이 v3를 기준으로 한다. **Sign-off**.
