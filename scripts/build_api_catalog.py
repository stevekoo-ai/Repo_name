...
def build() -> str:
    ...
    A("")
    A("**⚠️ 원래 문제가 두 개였다. 섞어 보면 진단이 어긋난다.**")
    A("")
    A("1. **통계표 ID가 틀렸다(2026-09-07 시점).** 프로브 실측에서 데이터")
    A("   엔드포인트(statisticsParameterData.do)가 `\"해당 통계표가 존재하지")
    A("   않습니다\"`로 응답했다 — 서버에 닿았고 인증도 통과했으며 좌표만")
    A("   틀렸다는 뜻이다. 등록된 7개 시리즈가 이 이유로 전부 값을 못")
    A("   가져오고 있었다(2026-09-08 좌표 확정으로 3개는 해소 — 아래 표).")
    ...
    A("추측만 하고 있었다.")
    A("")
    A("**2026-09-08 연결 완전 두절이 잠깐 있었다**: 후보 14개(지표 7개 ×")
    A("후보 2개씩)를 처음 돌렸을 때 **하나도 빠짐없이 connect timeout**")
    A("(15초)이었다 — tblId가 뭐든 상관없이 TCP 연결 자체가 안 됐다. 바로")
    A("전날(09-07)엔 같은 엔드포인트가 정상 연결됐었다(원인 미상: kosis.kr")
    A("측 문제 / GitHub Actions IP 대역 차단 / 반복 진단 트래픽 — [미검증]).")
    A("`kosis_lookup.py`에 8초짜리 연결 사전확인(`_connectivity_check`)을")
    A("추가한 뒤 재시도하니 약 15분 뒤 연결이 복구돼 아래 실측이 나왔다 —")
