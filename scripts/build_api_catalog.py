...
LIVE_STATUS: dict[str, str] = {
    "fred": "✅ 정상",
    "ecos": "✅ 정상",
    "kosis": "🔴 2026-09-08 실측: 데이터 엔드포인트 14/14 전부 connect timeout — 좌표 문제와 별개로 지금 이 순간 연결 자체가 끊김(전날엔 연결 정상)",
...
def build() -> str:
    ...
    A("추측만 하고 있었다.")
    A("")
    A("**🔴 2026-09-08 좌표 확정 작업 중 실측**: 후보 14개(지표 7개 × 후보 2개씩)를")
    A("데이터 엔드포인트로 전부 찔러봤는데 **하나도 빠짐없이 connect timeout**")
    A("(15초)이었다. tblId가 뭐든 상관없이 TCP 연결 자체가 안 됐다는 뜻 — 좌표가")
    A("맞고 틀리고를 떠나 이 순간은 검증 자체가 불가능했다. 바로 전날(09-07)")
    A("같은 엔드포인트가 정상 연결돼 JSON 에러 응답(\"해당 통계표가 존재하지")
    A("않습니다\")을 받았던 것과 대조적 — 하루 사이 완전 연결 두절로 악화된")
    A("것으로 보인다(원인 미상: kosis.kr 측 문제 / GitHub Actions IP 대역 차단 /")
    A("이날 반복된 진단 트래픽으로 인한 일시적 차단, 셋 다 가능성 있고 [미검증]).")
    A("`kosis_lookup.py`에 8초짜리 연결 사전확인을 추가해, 다음 시도부터는")
    A("14개 후보를 다 돌리기 전에 연결 자체가 죽어 있는지부터 싸게 확인한다.")
    A("")
