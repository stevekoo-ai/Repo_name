...
LIVE_STATUS: dict[str, str] = {
    "fred": "✅ 정상",
    "ecos": "✅ 정상",
    "kosis": "⚠️ 통계표 ID 오류 — 서버·인증은 정상, 좌표만 틀림",
...
def build() -> str:
    ...
    A("")
    A("**⚠️ 현재 통계표 ID가 틀려서 전부 실패 중이다.** 2026-09-07 프로브 실측:")
    A("응답은 `\"해당 통계표가 존재하지 않습니다\"` — 서버는 살아 있고 인증도")
    A("통과했다. 코드 주석에 오래 적혀 있던 \"kosis.kr TCP 타임아웃 / 한국 IP")
    A("제한 의심\"은 **틀린 진단**이었다.")
    A("")
    A("**신규/정정 통계표 찾는 법**: `scripts/kosis_lookup.py --search <키워드>`")
    A("(2026-09-07 추가). 예전엔 \"KOSIS엔 외부에서 닿는 검색 API가 없다\"고")
    A("적혀 있었지만 통합검색(statisticsSearch.do)이 존재한다 — 그 잘못된 전제")
    A("때문에 통계표를 계속 추측만 하고 있었다.")
    A("")
    ...
    A("")
    A("**실측 함정**: `period` 필드의 `M13`은 월이 아니라 **연평균**이다. 안")
    A("거르면 매년 12월 뒤에 13번째 포인트가 끼어 시계열이 오염된다.")
    A("")
