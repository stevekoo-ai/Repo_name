...
PRESETS = {
    ...
    # 2026-07-26 저녁 위키 정정 사건("7월 유가 급등"을 차트 육안 판독으로
    # 추정하다 레벨과 속도를 혼동)의 재발 방지로 추가. 그런데 이 프리셋을
    # 작업 브랜치에만 추가하고 main에 반영하지 않아 2026-07-28까지 실제로는
    # 한 번도 동기화되지 않았던 게 뒤늦게 발견됨 — 이 커밋으로 main에 반영.
    "us_brent": ("fred", "DCOILBRENTEU", "브렌트유 현물(일별, $/배럴)", "검증됨(FRED 표준 series_id)"),
    "us_wti": ("fred", "DCOILWTICO", "WTI 현물(일별, $/배럴)", "검증됨(FRED 표준 series_id)"),
}
